    ...
    >
      ...
      <el-table-column width="100" align="center" label="操作">
        <template slot-scope="scope">
          <el-button
            type="text"
            size="mini"
            :disabled="check[scope.$index]==0||scope.row.rStatus==31||scope.row.rStatus==19"
      ...
      >
        <template slot-scope="scope">
          <span
            class="orderlink"
            @click="toOrderPage(scope.row,'/recover/constructionbudget')"
      ...
      >
        <template slot-scope="scope">
          <div class="tr" v-if="scope.row.rStatus==19">{{ scope.row.rTotalprice }}</div>
          <div class="tr" v-else>0</div>
        </template>
      </el-table-column>
      <el-table-column sortable
        width="100"
    ...
    exportTable() {
      ...
      try {
        this.$fileSaver.saveAs(
          ...
          `施工预算列表${getDate.getToday().starttime}.xlsx`
        );
      } catch (e) {
        // 错误处理方式
      }
    ...
    getList(val) {
      ...
      this.$api.Receipt.get(params).then(res => {
        if (res.data) {
          ...
          if (datas.length) {
            this.$api.Receipt.checkHSSGD(datas.slice(0, -1)).then(res => {
              if (res) {
      ...
      this.$confirm(
        ...
        .then(() => {
          this.$api.Receipt.reset({ div: v, rid: n.rId }).then(res => {
            if (res.code == 200) {
              this.$message.success(`${v == 1 ? "终止" : "重置"}成功`);
              this.getList();
            } else {
