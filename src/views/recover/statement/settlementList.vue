    ...
    >
      ...
      >
        <template slot-scope="scope">
          <span
            class="orderlink"
            @click="toOrderPage(scope.row,'/recover/statement/settlement',0)"
      ...
      <el-table-column sortable property="rItemnumparent" width="200" label="来源订单">
        <template slot-scope="scope">
          <span
            class="orderlink"
            @click="toOrderPage(scope.row,'/recover/recoverOrder',1)"
      ...
      >
        <template slot-scope="scope">
          <div class="tr">{{ $PublicJS.money(parseFloat((scope.row.rTotalprice).toPrecision(12)), 2) }}</div>
        </template>
      </el-table-column>
      <el-table-column sortable
        width="150"
    ...
    exportTable() {
      ...
      try {
        this.$fileSaver.saveAs(
          ...
          `回收商品结算列表${getDate.getToday().starttime}.xlsx`
        );
      } catch (e) {
        // 错误处理方式
      }
