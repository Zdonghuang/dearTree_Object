  ...
  <div>
    ...
    >
      ...
      <el-table-column sortable align="center" prop="gName">
        ...
        <template slot-scope="scope">
          <input
            ...
            class="ipt"
            readonly
            :disabled="disabled||$route.query.datas?true:false"
          />
      ...
      <el-table-column sortable align="center" property="PAmount" label="金额">
        <template slot-scope="scope" v-if="scope.row.cAttrvalue||scope.row.PAmount">
          <input
            type="text"
    ...
    <my-footer
      ...
      @opendialog="opendialog(form.status==2?1:0)"
      :hideBtn1="false"
      :hideBtn2="!disabled&&form.status!=4&&form.status!=5"
      :BtnText="form.status!=2?'确认提交':'财务红冲'"
    ></my-footer>
    <el-dialog
...
export default {
  ...
  created() {
    ...
    if (this.$route.query.datas) {
      this.pid = window.atob(this.$route.query.datas);
      this.getdata1();
    }
    if (this.cuid) {
  ...
  methods: {
    ...
    getdata1() {
      ...
      this.$api.Premoney.getPremoneyAndDtsItemList(params).then(res => {
        if (res.records) {
          ...
          item.map((item, i) => {
            if (i == 0) {
              this.form.pCuid = item.PD_CUSERID;
              this.form.rCuidv = item.PD_CUSERIDV;
              this.form.rRemark = item.P_RAMARK;
              this.form.rHandman = item.PD_HANDMAN;
              this.form.rHandmanv = item.PD_HANDMANV;
              this.form.banknamegh = item.PD_BANKNAMEGH;
        ...
        this.$confirm("此操作将红冲此单据, 是否继续?", "提示", {
          ...
          this.tableData.forEach((item, index) => {
            if (item.cAttrcode && item.PAmount) {
              ...
              if (val) obj.pdStatus = 4;
              if (this.$route.query.datas) {
                obj.pdPidparent = item.pdPid;
                delete obj.pdPid;
              }
    ...
    getBankAccountData(val) {
      if (val) {
        if (!this.tableData[this.rowindex].PAmount) {
          val.PAmount = "0";
        } else {
