<template>
  <div>
    ...
    <el-row :gutter="20">
      ...
      <el-col :xs="24" :sm="6">
        <el-input placeholder="收款账户" v-model="form.banknamegh" size="small" readonly>
    ...
    opendialog(val) {
      ...
      this.tableData.forEach((item, index) => {
        if (item.cAttrcode && item.PAmount) {
          ...
          obj.pdHandman = this.form.rHandman;
      ...
      this.$api.Premoney.save(param).then(res => {
        ...
        if (res.code === 200) {
          ...
          this.$router.push("/financeAdvanceList");
        } else {
          this.$message("提交失败");
        }
      });
