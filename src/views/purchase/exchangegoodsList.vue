    ...
    >
      ...
      <el-table-column sortable property="rItemnum" width="200" label="单据编号">
        <template slot-scope="scope">
          <span
            class="bluecolor"
            @click="gostatement(scope.row,'exchangegoodsDetail')"
      ...
      <el-table-column sortable property="rSumstr" label="换出总数量" align="center">
        <template slot-scope="scope">
          <span
      ...
      <el-table-column sortable property="rSumstr" label="换入总数量" align="center">
        <template slot-scope="scope">
          <span
            v-for="(item,i) in scope.row.rSumstr.split(',')"
            :key="i"
    ...
    exportTable() {
      ...
      try {
        this.$fileSaver.saveAs(
          ...
          `采购换货列表${getDate.getToday().starttime}.xlsx`
        );
      } catch (e) {
        // 错误处理方式
      }
    ...
    getList(val) {
      ...
      this.$api.Receipt.get(params).then(res => {
        res.data.records.map(item => {
          if (item.rStatus == 3) item.rStatus = 7;
          if (item.rStatus == 8) item.rStatus = 17;
        });
        if (val=='xlsx') {
          setTimeout(() => {
              this.exportTable();
