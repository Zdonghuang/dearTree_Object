    ...
    >
      ...
      <el-table-column property label="操作" width="180" align="center">
        <template slot-scope="scope">
          <span
            :class="scope.row.rApprovalstatus != 2 && scope.row.rBillingstatus != 1 ?'active':'disabled'"
            v-has="4051"
            @click="gostatement(scope.row.rApprovalstatus != 2 && scope.row.rBillingstatus != 1 ?scope.row:false,'THstatementDetail',0)"
          >收款</span>
        </template>
      </el-table-column>
      ...
      <el-table-column sortable property="rItemnum" width="200" label="单据编号">
        <template slot-scope="scope">
          <span
            class="bluecolor"
            @click="gostatement(scope.row,'THstatementDetail',0)"
      ...
      >
        <template slot-scope="scope">
          <span
            class="orderlink"
            @click="gostatement(scope.row,'warehouse/outbound/CGTHoutboundDetail',1)"
    ...
    exportTable() {
      ...
      try {
        this.$fileSaver.saveAs(
          ...
          `采购退货结算列表${getDate.getToday().starttime}.xlsx`
        );
      } catch (e) {
        // 错误处理方式
      }
