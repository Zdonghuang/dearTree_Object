    ...
    >
      ...
      <el-table-column property label="操作" width="180" align="center">
        <template slot-scope="scope">
          <span
            :class="scope.row.rApprovalstatus != 2 && scope.row.rStatus != 19 && scope.row.rStatus != 31 ?'active':'disabled'"
            v-has="4041"
            @click="gostatement(scope.row.rApprovalstatus != 2 && scope.row.rStatus != 19 && scope.row.rStatus != 31 ?scope.row:false,'CGstatementDetail',0)"
          >付款</span>
          <span
            :class="scope.row.rStatus != 31&&scope.row.rInoutstatus==1 ?'active':'disabled'"
            v-has="4021"
            @click="gostatement(scope.row.rStatus != 31&&scope.row.rInoutstatus==1?scope.row:'','returngoodsDetail',0)"
          >退货</span>
          <span
            :class="scope.row.rStatus != 31&&scope.row.rInoutstatus==1 ?'active':'disabled'"
            v-has="4031"
            @click="gostatement(scope.row.rStatus != 31&&scope.row.rInoutstatus==1?scope.row:'','exchangegoodsDetail',0)"
          >换货</span>
        </template>
      </el-table-column>
      ...
      <el-table-column sortable property="rItemnum" width="200" label="单据编号">
        <template slot-scope="scope">
          <span
            class="bluecolor"
            @click="gostatement(scope.row,'CGstatementDetail',0)"
      ...
      >
        <template slot-scope="scope">
          <span
            class="orderlink"
            @click="gostatement(scope.row,'quotationsheetDetailnext',1)"
      ...
      <el-table-column sortable property="rInoutstatus" label="入库状态">
        <template slot-scope="scope">
          <el-tag
            effect="dark"
            :type="`${scope.row.rInoutstatus}` | rInoutclass"
            size="mini"
          >{{ scope.row.rInoutstatus | rInoutstatus }}</el-tag>
        </template>
      </el-table-column>
    ...
    exportTable() {
      ...
      try {
        this.$fileSaver.saveAs(
          ...
          `采购结算列表${getDate.getToday().starttime}.xlsx`
        );
      } catch (e) {
        // 错误处理方式
      }
