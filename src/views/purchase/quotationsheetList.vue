      ...
      <el-col :xs="24" :sm="12" align="right">
        <el-button-group v-if="!$router.history.current.query.from">
          <el-button
            ...
            icon="el-icon-plus"
            type="warning"
            @click="$router.push({path: 'quotationsheetDetail'})"
            v-has="4071"
          >新增</el-button>
    ...
    >
      ...
      <!-- <el-table-column align="center" label="操作"  width="100" >
        <template slot-scope="scope">
          <span
            :class="scope.row.rStatus == 30 ?'active':'disabled'"
            @click="gostatement(scope.row.rStatus == 30 ?scope.row:false,'quotationsheetDetail')"
          >修改</span>
          <span
            :class="scope.row.rStatus != 19 ?'active':'disabled'"
            @click="scope.row.rStatus != 19&&$router.push('/finance/financePay')"
          >预付</span>
        </template>
      </el-table-column>-->
      ...
      <el-table-column sortable property="rItemnum" width="200" label="单据编号" align="center">
        <template slot-scope="scope">
          <span
            class="bluecolor"
            @click="gostatement(scope.row,'quotationsheetDetail')"
    ...
    exportTable() {
      ...
      try {
        this.$fileSaver.saveAs(
          ...
          `采购申请列表${getDate.getToday().starttime}.xlsx`
        );
      } catch (e) {
        // 错误处理方式
      }
