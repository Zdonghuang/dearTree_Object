    ...
    >
      ...
      <el-table-column sortable property="rItemnum" width="200" label="单据编号" align="center">
        <template slot-scope="scope">
          <span
            class="bluecolor"
            @click="gostatement(scope.row,'CKstatementDetail',0)"
      ...
      >
        <template slot-scope="scope">
          <span
            class="orderlink"
            @click="gostatement(scope.row,'RKstatementDetail',1)"
      ...
      <el-table-column sortable property="rStatus" label="单据状态" align="center">
        <template slot-scope="scope">
          <el-tag
            ...
            size="mini"
            v-if="scope.row.rStatus!=32"
          >{{ scope.row.rStatus | orderStatus }}</el-tag>
          <el-tag effect="dark" type="danger" size="mini" v-if="scope.row.rStatus==32">待记账</el-tag>
        </template>
      </el-table-column>
      ...
      <!-- <el-table-column sortable property="rApprovalstatus" label="审核状态">
        <template slot-scope="scope">
          <el-tag
            effect="dark"
      ...
      <el-table-column sortable property="rTotalprice" label="本单金额" align="center">
        <template slot-scope="scope">{{scope.row.rTotalprice}}</template>
      </el-table-column>
...
export default {
  ...
  methods: {
    exportTable() {
      ...
      try {
        this.$fileSaver.saveAs(
          ...
          `调拨出库结算列表${getDate.getToday().starttime}.xlsx`
        );
      } catch (e) {
        // 错误处理方式
      }
    ...
    getList(val) {
      ...
      if (!params.rWhidv) delete params.rWhidv;
      if (!params.rWhid) delete params.rWhid;
