<template>
  <div>
      ...
      <el-col :xs="24" :sm="12" align="right">
        <el-button-group v-if="!$router.history.current.query.from">
          <el-button
            ...
            type="warning"
            icon="el-icon-plus"
            @click="$router.push('/lease/leaseOrder')"
            v-has="1571"
          >新增</el-button>
      ...
      <el-col :sm="4" :xs="24">
        ...
        >
          ...
          >
            ...
            <span style="float: left; width:320px">{{ item.label }}</span>
          </el-option>
        </el-select>
      </el-col>
      <el-col :xs="24" :sm="4">
    ...
    >
      ...
      <el-table-column sortable width="220" align="center" label="操作">
        <template slot-scope="scope">
          ...
          <el-button
            type="text"
            size="mini"
            :disabled="scope.row.rStatus!=16"
            @click="toOrderPage(scope.row,'XZ')"
            v-has="1571"
          >续租</el-button>
          <el-button
            type="text"
            size="mini"
            :disabled="scope.row.rStatus!=16"
            @click="toOrderPage(scope.row,'TZ')"
            v-has="1571"
          >退租</el-button>
          <el-button type="text" size="mini" @click="toOrderPage(scope.row,'YSK')">预收</el-button>
          <!-- <el-button
            type="text"
            size="mini"
            :disabled="scope.row.rStatus!=19 && scope.row.rBillingstatus!=1"
            @click="toOrderPage(scope.row,'TYJ')"
            v-has='1571'
          >退押金</el-button>-->
        </template>
      </el-table-column>
      ...
      <el-table-column sortable align="center" show-overflow-tooltip property="months" label="租赁月份"></el-table-column>
      ...
      <!-- <el-table-column sortable
        ...
        align="center"
        show-overflow-tooltip
        property="rReceivableyj"
        label="押金"
      >
      <template slot-scope="scope" v-if="scope.row.rReceivableyj">
          <div class="tr">{{ $PublicJS.money(scope.row.rReceivableyj, 2) }}</div>
        </template>
      </el-table-column>-->
      <el-table-column sortable
        width="120"
      ...
      <el-table-column sortable align="center" show-overflow-tooltip property="rHandmanv" label="签单人"></el-table-column>
      <el-table-column sortable align="center" show-overflow-tooltip property="rHandmanv" label="制单人"></el-table-column>
...
export default {
  ...
  created() {
    ...
    this.getList();
    // // 客户
    // this.$api.Customer.get({ div: 16, size: 9999, auth: 1, querydiv: 1 }).then(
    //   res => {
    //     if (res.code == 200) {
  ...
  methods: {
    ...
    toOrderPage(val, e) {
      if (e) {
        if (e == "BHD") {
          this.$confirm("此操作将生成租赁备货单, 是否继续?", "提示", {
            confirmButtonText: "确定",
            cancelButtonText: "取消",
            type: "warning"
          })
            .then(() => {
              this.$api.Rent.createBhd({ receiptId: val.rId }).then(res => {
                if (res.code === 200) {
                  this.$message({
                    message: "生成租赁备货单成功",
                    type: "success"
        ...
        if (e == "TZ") {
          this.$confirm("此操作将退租, 是否继续?", "提示", {
            ...
            .then(() => {
              this.$api.Rent.withdrawal({ receiptId: val.rId }).then(res => {
                if (res.code === 200) {
                  this.$message({
                    message: "退租成功，等待入库",
                    type: "success"
        ...
        //   this.$router.push({
        //     path: "/deposit",
        //     query: { rID:val.rId, ID: val.rCuid }
        //   });
        // }
        if (e == "YSK") {
          this.$router.push({
    ...
    exportTable() {
      ...
      try {
        this.$fileSaver.saveAs(
          ...
          `租赁列表${getDate.getToday().starttime}.xlsx`
        );
      } catch (e) {
        // 错误处理方式
      }
