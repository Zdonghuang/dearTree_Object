<template>
  <div>
    ...
    <el-row :gutter="20">
      ...
      <el-col :sm="4" :xs="24">
        <el-select v-model="form.approvalstatus" size="small" class="selectSlot4" clearable>
          <template slot="prefix">
    ...
    >
      ...
      <el-table-column
        sortable
        align="center"
        property="PD_AMOUNTTOTAL"
        label="金额"
        show-overflow-tooltip
      >
        <template slot-scope="scope">
          <div class="tr">{{$PublicJS.money(scope.row.PD_AMOUNTTOTAL, 2) }}</div>
        </template>
      </el-table-column>
      <el-table-column
        sortable
        align="center"
        property="PD_HANDMANV"
        label="经手人"
        show-overflow-tooltip
      ></el-table-column>
      <el-table-column
...
export default {
  ...
  methods: {
    ...
    exportTable() {
      ...
      try {
        this.$fileSaver.saveAs(
          ...
          `预收减少列表${getDate.getToday().starttime}.xlsx`
        );
      } catch (e) {
        // 错误处理方式
      }
    ...
    // 获取列表
    getPremoneyList(val) {
      this.form.cuid = this.form.rCuid;
