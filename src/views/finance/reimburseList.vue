<template>
  <div>
    ...
    >
      ...
      <el-table-column sortable align="center" property="PD_AMOUNTTOTAL" label="报销金额" show-overflow-tooltip>
        <template slot-scope="scope">
...
export default {
  ...
  methods: {
    tofinanceAdd() {
      this.$router.push("/reimburseOrder");
    },
    // 合计
    getSummaries(param) {
    ...
    exportTable() {
      ...
      try {
        this.$fileSaver.saveAs(
          ...
          `报销列表${getDate.getToday().starttime}.xlsx`
        );
      } catch (e) {
        // 错误处理方式
      }
