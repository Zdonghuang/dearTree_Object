...
export default {
  ...
  methods: {
    ...
    Printing() {
      ...
      window.open(`/#/printingJK?data=${data}`, "_blank");
    },
    openlink(url) {
      ...
      this.$api.Premoney.getPremoneyAndDtsItemList(params).then(res => {
        if (res.records) {
          ...
          item.map((item, i) => {
            ...
            this.form.TPremoneydtsList[0].pdApprovalstatus = item.PD_APPROVALSTATUS;
            this.form.TPremoneydtsList[0].pdStatus = item.PD_STATUS;
