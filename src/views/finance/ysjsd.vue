...
<template>
  <div>
    ...
    <el-row :gutter="20">
      ...
      <el-col :xs="24" :sm="6">
        <el-input placeholder="预收余额" v-model="pFamount" size="small" readonly disabled>
      ...
      <el-col :xs="24" :sm="6">
        <el-input
          ...
          size="small"
          readonly
          :disabled="form.PD_APPROVALSTATUS!=2"
        >
    ...
    <my-footer
      ...
      @opendialog="opendialog(form.PD_APPROVALSTATUS==2?2:1)"
      :hideBtn1="false"
      :hideBtn2="!disabled"
    ...
    >
      ...
      <examine :rtype="88" @close="submit" @openuser="openuser" :ID="dingding" :ID1="dingding1"></examine>
    </el-dialog>
  </div>
</template>
...
params.auth=3
      ...
      this.$api.Premoney.getPremoneyAndDtsItemList(params).then(res => {
        if (res.records) {
          ...
          items.map((item, i) => {
            if (i == 0) {
              this.form = item;
              this.form.cuid = item.PD_CUID;
              this.cuid = item.PD_CUID;
              this.getC();
              this.form.cuidv = item.PD_CUIDV;
    ...
    opendialog(val) {
      ...
      if (!this.form.banknamegh && this.form.PD_APPROVALSTATUS == 2) {
        this.$message({ message: "请选择银行账户", type: "warning" });
        return false;
      }
      let n = 0;
    ...
    setmoney(v) {
      ...
      if (v.PAmount > this.pFamount) {
        v.PAmount = 0;
        return this.$message.error("不能超出客户预收余额");
      }
    },
    searchbox(e, val) {
      this.rowindex = val;
