<template>
  <div>
    ...
    >
      ...
      >
        <template slot-scope="scope">
          <span class="orderlink" @click="toOrderPage(scope.row,0)">{{ scope.row.rItemnum }}</span>
        </template>
      </el-table-column>
      <el-table-column sortable
        width="180"
      ...
      >
        <template slot-scope="scope">
          <span class="orderlink" @click="toOrderPage(scope.row,1)">{{ scope.row.rItemnumparent }}</span>
        </template>
      </el-table-column>
...
export default {
  ...
  data() {
    return {
      ...
      rWhidOptions: [],
      rHandmanOptions: [],
      rStatusOptions:[{label:'待出库',value:7},{label:'单据完结',value:19}],
      form: {
        rHandmanv: "",
        rHandman: "",
        rWhidv: "",
  ...
  methods: {
    ...
    toOrderPage(val, n) {
      let paths;
      if (!n) {
        paths = "/warehouse/outbound/XStransferoutboundDetail";
        this.$router.push({
          path: paths,
          query: { data: window.btoa(val.rId) }
        });
      } else {
        paths = "/warehouse/transfer/XStransferApplication";
        this.$router.push({
          path: paths,
    ...
    exportTable() {
      ...
      try {
        this.$fileSaver.saveAs(
          ...
          `分公司调拨出库列表${getDate.getToday().starttime}.xlsx`
        );
      } catch (e) {
        // 错误处理方式
      }
