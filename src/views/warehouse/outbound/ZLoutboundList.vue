    ...
    >
      ...
      <!-- <el-table-column width="100" align="center"  label="操作">
        <template slot-scope="scope">
          <el-button type="text" size="mini" :disabled="scope.row.rStatus!=7"  @click="operating(scope.row,'reset')" v-has='361'>重置</el-button>
          <el-button type="text" size="mini" :disabled="scope.row.rStatus!=7" @click="operating(scope.row,'cancel')" v-has='361'>作废</el-button>
        </template>
      </el-table-column> -->
      ...
      <el-table-column sortable
        ...
        label="附件图片"
      >
        <template slot-scope="scope" v-if="scope.row.rFile">
          <el-popover
            placement="right"
            trigger="hover"
            v-for="(url,i) in JSON.parse(scope.row.rFile)"
            :key="i"
          >
...
export default {
  ...
  data() {
    return {
      ...
      rWhidOptions: [],
      rHandmanOptions: [],
      rStatusOptions:[{label:'已出库',value:8},{label:'待出库',value:7}],
      form: {
        rCuidv: "",
        rCuid: "",
        rHandmanv: "",
  ...
  methods: {
    ...
    toOrderPage(val, e) {
      if (e) {
        if (e == "YD") {
          this.$router.push({
            path: "/warehouse/distribution/leaseDistributionDetail",
    ...
    operating(val, e) {
      ...
      this.$confirm("此操作将"+msg+", 是否继续?", "提示", {
        ...
        .then(() => {
          this.$api.Sale[apis]({ receiptId: val.rId }).then(res => {
            if (res.code === 200) {
    ...
    exportTable() {
      ...
      try {
        this.$fileSaver.saveAs(
          ...
          `租赁出库列表${getDate.getToday().starttime}.xlsx`
        );
      } catch (e) {
        // 错误处理方式
      }
