<template>
  <div>
    ...
    <el-row :gutter="20" class="mbottom">
      ...
      <el-col :xs="24" :sm="6">
        <el-input placeholder="请输入运单号" v-model="form.rShipmentnumber" size="small">
      ...
      <el-col :xs="24" :sm="24">
        <el-button type="warning" v-if="form.rStatus==19" @click="updataFUN" size="mini">更新</el-button>
      </el-col>
      <el-col :xs="24" :sm="24">
    ...
    getOrder() {
      ...
      this.$api.Receipt.get(rId).then(res => {
        if (res.data) {
          ...
          // 施工环境回显
          this.$api.Receipt.gettconstruct({ rid: items.rRootid }).then(res => {
            if (res.data) {
        ...
        .then(() => {
          ...
          this.$ajax.post("/system/upload", this.formData).then(res => {
            if (res.data.code == 200) {
              ...
              this.form.rDate = this.form.rDate.replace(
                new RegExp("-", "g"),
                ""
              );
    ...
    dialogSubmit(val) {
      ...
      this.$ajax.post("/system/upload", this.formData).then(res => {
        if (res.data.code == 200) {
          ...
          this.$api.Sale.outbound(this.form).then(res => {
            loading.close();
            if (res.code == 200) {
              this.$message({
                type: "success",
