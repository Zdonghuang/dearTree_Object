  ...
  <div>
    <el-row :gutter="20" class="titleBox">
      ...
      <el-col :xs="24" :sm="8" align="center">
        <p class="title">{{url?'内部调拨单':'分公司调拨单'}}</p>
      </el-col>
      <el-col :xs="24" :sm="8" align="right">
        <el-button-group v-if="!$router.history.current.query.from">
          ...
          <el-button size="small" icon="el-icon-printer" @click="Printing" v-has="370">打印</el-button>
        </el-button-group>
      </el-col>
    </el-row>
    <el-row :gutter="20">
    ...
    <el-dialog
      ...
      width="60%"
    >
      <selectWarehouse @emitWarehouseData="getOUTWarehouseData" :oid="form.rOid"></selectWarehouse>
    </el-dialog>
    <el-dialog
      title="入库仓库选择"
      :close-on-click-modal="false"
      ...
      width="60%"
    >
      <selectWarehouse @emitWarehouseData="getINWarehouseData" :oid="form.rOid" :wid="form.rWhid"></selectWarehouse>
    </el-dialog>
    <el-dialog
      title="职员选择"
      :close-on-click-modal="false"
...
export default {
  ...
  data() {
    return {
      url:
        this.$route.path.split("/")[3] == "transferApplication" ? true : false,
      tableData: [],
      icon: "",
  ...
  methods: {
    ...
    dialogSubmit(val, val1) {
      ...
      this.$api.Receipt.saveall(params).then(res => {
        ...
        if (res.code === 200) {
          ...
          if (this.url) {
            this.$router.push("/warehouse/transfer/transferList");
          } else {
            this.$router.push("/warehouse/transfer/XStransferList");
          }
        } else {
    ...
    Printing() {
      ...
      window.open(
        `/#/warehouse/transfer/printingTransfer?data=${data}`,
        "_blank"
      );
    },
    // 合计
