<template>
  <div>
    <el-row :gutter="20" class="titleBox">
      ...
      <el-col :xs="24" :sm="12" align="right">
        <el-button-group v-if="!$router.history.current.query.from">
          <el-button size="small" type="warning" icon="el-icon-plus" @click="toOrderPage(0)" v-has='370'>新增</el-button>
    ...
    >
      ...
      <el-table-column sortable property="rTotalprice" align="center" label="成本金额"></el-table-column>
      <el-table-column sortable property="rHandmanv" align="center" label="经手人"></el-table-column>
      <el-table-column sortable property="rOidv" align="center" label="所属公司"></el-table-column>
...
export default {
  ...
  methods: {
    toOrderPage(val) {
      let path = "";
      if (this.url) {
        path = "/warehouse/transfer/transferApplication";
      } else {
        path = "/warehouse/transfer/XStransferApplication";
      }
      if (val) {
        this.$router.push({
          path: path,
    ...
    exportTable() {
      ...
      try {
        this.$fileSaver.saveAs(
          ...
          `内部调拨列表${getDate.getToday().starttime}.xlsx`
        );
      } catch (e) {
        // 错误处理方式
      }
