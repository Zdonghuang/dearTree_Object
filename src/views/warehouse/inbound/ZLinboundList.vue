    ...
    >
      ...
      <el-table-column width="100" align="center"  label="操作">
        <template slot-scope="scope">
          <el-button type="text" size="mini" :disabled="scope.row.rStatus!=7"  @click="operating(scope.row,'reset')" v-has='362'>重置</el-button>
          <el-button type="text" size="mini" :disabled="scope.row.rStatus!=7" @click="operating(scope.row,'cancel')" v-has='362'>作废</el-button>
        </template>
      </el-table-column>
...
export default {
  ...
  data() {
    return {
      ...
      rWhidOptions: [],
      rHandmanOptions: [],
      rStatusOptions:[{label:'未入库',value:17},{label:'已入库',value:18}],
      form: {
        rCuidv: "",
        rCuid: "",
        rHandmanv: "",
  ...
  methods: {
    ...
    toOrderPage(val, e) {
      if (e) {
        ...
        this.$router.push({
          path: "/warehouse/inbound/ZLinboundDetail",
    ...
    exportTable() {
      ...
      try {
        this.$fileSaver.saveAs(
          ...
          `租赁入库列表${getDate.getToday().starttime}.xlsx`
        );
      } catch (e) {
        // 错误处理方式
      }
