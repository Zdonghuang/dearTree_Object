...
export default {
  ...
  methods: {
    ...
    toOrderPage(val, n) {
      let path;
      if (n) {
        path = "/warehouse/outbound/XStransferoutboundDetail";
        this.$router.push({
          path: path,
          query: { data: window.btoa(val.rIdparent) }
        });
      } else {
        path = "/warehouse/inbound/XStransferInboundDetail";
        this.$router.push({
          path: path,
    ...
    exportTable() {
      ...
      try {
        this.$fileSaver.saveAs(
          ...
          `分公司调拨入库列表${getDate.getToday().starttime}.xlsx`
        );
      } catch (e) {
        // 错误处理方式
      }
