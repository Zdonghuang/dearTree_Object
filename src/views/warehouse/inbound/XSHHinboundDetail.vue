    ...
    >
      ...
      >
        <template slot-scope="scope" v-if="scope.row.gItemnum">
          <input type="text" v-model="scope.row.iSellingprice" :class="defstyle" />
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    dialogSubmit(val) {
      ...
      // 保存换货入库单
      this.$api.Exchange.warehousing(params).then(res => {
        loading.close();
        if (res.code === 200) {
          this.$message({ message: "提交成功", type: "success" });
          this.$router.push("/warehouse/inbound/saleExchangeInboundList");
        } else {
