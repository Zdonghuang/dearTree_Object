<template>
  <div>
    ...
    >
      ...
      <el-table-column sortable align="center" property="gName" width="200" >
      ...
      <el-table-column sortable align="center" property="gRemark" label="备注"></el-table-column>
    </el-table>
    <br />
    <br />
    <div class="bigbox" v-if="form.rStatus==19||form.rType==41">
      <div class="litbox">
        <span>单据完结</span>
      </div>
    </div>
      ...
      this.$api.Receipt.get(rId).then(res => {
        if (res.data) {
          ...
          this.$api.Receiptdts.getReceiptDtsGoods({
            ...
            this.loading.close();
            res.data.map(item => {
              if (item.rdIotype == 1) return false;
    ...
    dialogSubmit() {
      ...
      this.$api.Receipt.rkAll(this.form).then(res => {
        ...
        if (res.code == 200) {
          ...
          this.$router.push("/warehouse/inbound/purchaseExchangeInboundList");
        } else {
