  ...
  <div>
    ...
    <el-row :gutter="20">
      ...
      <el-col :sm="2" :xs="24">
       ...
       <el-col :sm="4" :xs="24">
         <div class="s_box" style="padding-left:0">
          <el-checkbox
    ...
    >
      ...
      <el-table-column sortable width="150" align="center" property="rApprovalstatus" label="审批状态">
        <template slot-scope="scope">
          <el-tag
            effect="dark"
      ...
      <el-table-column sortable width="150" align="center" show-overflow-tooltip property label="盘点数">
        <template slot-scope="scope">
          <div>{{ scope.row.rSumstr.split(':')[1] }}</div>
        </template>
      </el-table-column>
      <el-table-column sortable width="150" align="center" show-overflow-tooltip property label="账面数量">
        <template slot-scope="scope">
          <div>{{ scope.row.rSumstr.split(':')[2] }}</div>
        </template>
      </el-table-column>
      <el-table-column sortable width="150" align="center" show-overflow-tooltip property label="盘点差异数">
        <template slot-scope="scope">
          <!-- <div v-if="scope.row.rSumstr.split(':')[2] < scope.row.rSumstr.split(':')[1] ">
         盘盈 {{ scope.row.rSumstr.split(':')[1] - scope.row.rSumstr.split(':')[2] }}
        </div>
        <div v-if="scope.row.rSumstr.split(':')[2] > scope.row.rSumstr.split(':')[1] ">
         盘亏 {{ scope.row.rSumstr.split(':')[2] - scope.row.rSumstr.split(':')[1] }}
        </div>
        <div v-if="scope.row.rSumstr.split(':')[2] == scope.row.rSumstr.split(':')[1] ">
         0
          </div>-->
          <div>{{ scope.row.rSumstr.split(':')[1] - scope.row.rSumstr.split(':')[2] }}</div>
        </template>
      </el-table-column>
      <el-table-column sortable
        width="150"
      ...
      >
        <template slot-scope="scope">
          <div>
            <span v-if="scope.row.rTotalprice<0">-</span>
            {{ $PublicJS.money(Math.abs(scope.row.rTotalprice), 2) }}
          </div>
        </template>
      </el-table-column>
      <el-table-column sortable
...
export default {
  ...
  methods: {
    // 页面跳转
    toOrderPage(val, e) {
      let paths = "/warehouse/inventory/fullDetail";
      if (val.rItemnum) {
        this.$router.push({
          path: paths,
    ...
    exportTable() {
      ...
      try {
        this.$fileSaver.saveAs(
          ...
          `盘点列表${getDate.getToday().starttime}.xlsx`
        );
      } catch (e) {
        // 错误处理方式
      }
