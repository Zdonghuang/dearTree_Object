      ...
      <!-- <el-table-column width="100" align="center"  label="操作">
        <template slot-scope="scope">
          <el-button
            type="text"
            size="mini"
            :disabled="scope.row.rApprovalstatus!=3 "
            @click="toOrderPage(scope.row,'CK')"
          >出库</el-button>
        </template>
      </el-table-column>-->
...
export default {
  ...
  methods: {
    // 页面跳转
    toOrderPage(val, e) {
      let paths = "/warehouse/inventory/scrappedDetail";
      if (e == "CK") {
        paths = "/warehouse/outbound/BFoutboundList";
      }
      if (val.rItemnum) {
        this.$router.push({
          path: paths,
    ...
    exportTable() {
      ...
      try {
        this.$fileSaver.saveAs(
          ...
          `报废列表${getDate.getToday().starttime}.xlsx`
        );
      } catch (e) {
        // 错误处理方式
      }
