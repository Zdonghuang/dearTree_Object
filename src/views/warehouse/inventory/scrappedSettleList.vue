      ...
      <el-table-column width="100" align="center" label="操作">
        <template slot-scope="scope">
          <el-button
            ...
            v-has="367"
            v-if="scope.row.rStatus!=32 && scope.row.rApprovalstatus!=3"
          >结算</el-button>
          <el-button
            type="text"
            size="mini"
      ...
      <el-table-column sortable width="100" align="center" property="rStatus" label="单据状态">
        <template slot-scope="scope">
          <el-tag
            ...
            v-if="form.rApprovalstatus!=3 && form.rStatus!=32"
            size="mini"
...
export default {
  ...
  methods: {
    // 页面跳转
    toOrderPage(val, e) {
      let paths = "/warehouse/inventory/scrappedSettleDetail";
      let querys = { data: window.btoa(val.rId) };
      if (e == "YD") {
        paths = "/warehouse/outbound/BFoutboundDetail";
    ...
    exportTable() {
      ...
      try {
        this.$fileSaver.saveAs(
          ...
          `报废结算列表${getDate.getToday().starttime}.xlsx`
        );
      } catch (e) {
        // 错误处理方式
      }
