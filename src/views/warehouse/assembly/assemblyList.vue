<template>
  <div>
    <el-row :gutter="20" class="titleBox">
      ...
      <el-col :xs="24" :sm="12" align="right">
        <el-button-group v-if="!$router.history.current.query.from">
          <el-button size="small" type="warning" icon="el-icon-plus" @click="toOrderPage" v-has='368'>新增</el-button>
    ...
    >
      <el-table-column sortable width="80" align="center" type="index" label="序号"></el-table-column>
      <!-- <el-table-column sortable width="150" align="center"  label="操作">
      ...
      <el-table-column sortable
        ...
        label="仓库"
      ></el-table-column>
      <el-table-column sortable property="rSumstr" label="出库数量" align="center">
        <template slot-scope="scope">
          <span
            v-for="(item,i) in scope.row.rSumstr.split(',')"
            :key="i"
          >{{item.split(':')[0]==1?item.split(':')[1]:''}}</span>
        </template>
      </el-table-column>
      <el-table-column sortable property="rSumstr" label="入库数量" align="center">
        <template slot-scope="scope">
          <span
            v-for="(item,i) in scope.row.rSumstr.split(',')"
            :key="i"
...
export default {
  ...
  methods: {
    // 页面跳转
    toOrderPage(val, e) {
      let paths = "/warehouse/assembly/assemblyOrder";
      if (e == "CK") {
        paths = "/warehouse/outbound/ZZoutboundList";
      }
      if (e == "RK") {
        paths = "/warehouse/inbound/ZZinboundList";
      }
      if (val.rItemnum) {
        this.$router.push({
          path: paths,
    ...
    exportTable() {
      ...
      try {
        this.$fileSaver.saveAs(
          ...
          `组装列表${getDate.getToday().starttime}.xlsx`
        );
      } catch (e) {
        // 错误处理方式
      }
