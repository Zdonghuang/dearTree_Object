      ...
      <el-table-column sortable width="120" align="center" label="操作">
        <template slot-scope="scope">
          <el-button
            ...
            @click="operating(scope.row,'Return')"
            v-has="1079"
          >退货</el-button>
          <el-button
            type="text"
            size="mini"
            :disabled="scope.row.rStatus!=19"
            @click="operating(scope.row,'Exchange')"
            v-has="1079"
          >换货</el-button>
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    // 页面跳转
    toOrderPage(val, e) {
      let paths = "/warehouse/outbound/XSoutboundDetail";
      let querys = { data: window.btoa(val.rId) };
      if (e == "YD") {
        paths = "/sales/saleOrder";
    ...
    operating(val, e) {
      let paths = "";
      if (e == "Return") {
        paths = "/sales/saleReturnOrder";
      }
      if (e == "Exchange") {
        paths = "/sales/saleReplaceOrder";
      }
      this.$router.push({
        path: paths,
    ...
    exportTable() {
      ...
      try {
        this.$fileSaver.saveAs(
          ...
          `销售退换货申请列表${getDate.getToday().starttime}.xlsx`
        );
      } catch (e) {
        // 错误处理方式
      }
