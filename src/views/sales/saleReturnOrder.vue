    ...
    <el-row :gutter="20" class="titleBox">
      ...
      <el-col :xs="24" :sm="8" align="right">
        <el-button-group v-if="!$router.history.current.query.from">
          ...
          <el-button size="small" icon="el-icon-printer" @click="Printing" v-has='1029'>打印</el-button>
        </el-button-group>
      </el-col>
    </el-row>
    <el-row :gutter="20">
        ...
        <el-row>
          ...
          <el-col :xs="24" :sm="6" align="right">
            <div class="toolright" v-if="tableData.length">
              <div class="jizhang" @click="btnSubmit" v-if="form.rStatus==19 ">
                <p style="padding:0 30px">退货</p>
              </div>
            </div>
          </el-col>
        </el-row>
...
export default {
  ...
  created() {
    ...
    if(this.$route.query.BH){
      this.orderID = this.$route.query.BH;
      this.getOrder();
    }
  },
  computed: {
  ...
  methods: {
    ...
    Printing() {
      ...
      window.open(`/#/sales/printingReturnOrder?data=${data}`, "_blank");
    },
    // 合计
    getSummaries(param) {
    ...
    dialogSubmit() {
      ...
      // 保存退货单
      this.$api.Refund.save(params).then(res => {
        if (res.code === 200) {
          this.$message({ message: "提交成功", type: "success" });
          this.$router.push("/sales/saleReturnOrderList");
        } else {
