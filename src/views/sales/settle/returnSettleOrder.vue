  ...
  <div>
    ...
    <el-row :gutter="20" class="mtop15 mbottom">
      <el-col :xs="24" :sm="6" v-if="form.rStatus!=19">
        <el-input placeholder="0" v-model="receiptFinancial.FMpFamount" size="small" disabled>
      ...
      <el-col :xs="24" :sm="6" v-if="form.rStatus!=19">
        ...
        >
          <template slot="prepend">启用预付款(￥)</template>
        </el-input>
      </el-col>
      <el-col :xs="24" :sm="6" v-if="form.rStatus!=19">
        <el-input placeholder="0" v-model="sumYH" size="small" disabled>
        ...
        <el-row>
          ...
          <el-col :xs="24" :sm="6" align="right">
            ...
            <div class="toolright" v-if="hideAllBtn">
              <div class="baocun" @click="btnSubmit(0)">
                <p style="padding:0 30px">转到预收</p>
              </div>
              <div class="jizhang" @click="btnSubmit(1)">
...
export default {
  ...
  computed: {
    ...
    sumYH: function() {
      return (this.form.rTotalprice - this.receiptFinancial.amount).toFixed(2);
    }
  },
  methods: {
    // 单据回显
        ...
        if (res.data) {
          ...
          if (items.rStatus == 19) {
            ...
            this.receiptFinancial.amount =
              items.rTotalprice - this.receiptFinancial.rDiscountamount;
          } else {
          ...
          this.$api.Premoney.get({ cuid: items.rCuid }).then(res => {
            if (res.data.length) {
              ...
              if (items1.pFamount < items.rTotalprice) {
    ...
    btnSubmit(val) {
      ...
      if (!val) {
        ...
        if (this.receiptFinancial.amount < this.form.rTotalprice) {
    ...
    dialogSubmit(val) {
      ...
      if (!val) {
        ...
        param.billingDiv = 2;
        if (this.receiptFinancial.amount < this.receiptFinancial.pFamount) {
          param.billingAmount = this.receiptFinancial.amount;
        } else {
          param.billingAmount = this.receiptFinancial.pFamount;
        }
      }
      this.$api.Receipt.saveall(param).then(res => {
        ...
        if (res.code === 200) {
          ...
          this.$router.push("/sales/settle/returnSettleOrderList");
        } else {
    ...
    iptBlur(val) {
      if (!isNaN(val)) {
        if (!val) {
          ...
          this.receiptFinancial.amount = this.receiptFinancial.pFamount;
          this.$message("付款金额不得小于0");
        } else if (val > this.receiptFinancial.pFamount) {
          // if (this.receiptFinancial.pFamount < this.form.rTotalprice) {
          //   this.receiptFinancial.amount = this.receiptFinancial.pFamount;
          // } else {
          //   this.receiptFinancial.amount = this.form.rTotalprice;
          // }
          this.receiptFinancial.amount = this.receiptFinancial.pFamount;
          this.$message("付款金额不得大于预付金额");
        }
      } else {
        this.receiptFinancial.amount = this.receiptFinancial.pFamount;
        this.$message("请输入数字");
