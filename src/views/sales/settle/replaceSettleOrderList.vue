  ...
  <div>
    ...
    >
      ...
      <el-table-column width="100" align="center" label="操作">
        <template slot-scope="scope">
          <el-button
            type="text"
            size="mini"
            :disabled="scope.row.rBillingstatus==1"
      ...
      >
        <template slot-scope="scope">
          <div class="tr">
            <b v-if="scope.row.rTotalprice<0">-</b>
  ...
  methods: {
    ...
    toOrderPage(val, e) {
      if (e) {
        if (e == "YD") {
          this.$router.push({
            path: "/sales/saleReplaceOrder",
            query: { data: window.btoa(val.rIdparent) }
          });
        }
      } else {
        this.$router.push({
          path: "/sales/settle/replaceSettleOrder",
    ...
    exportTable() {
      ...
      try {
        this.$fileSaver.saveAs(
          ...
          `销售换货结算列表${getDate.getToday().starttime}.xlsx`
        );
      } catch (e) {
        // 错误处理方式
      }
