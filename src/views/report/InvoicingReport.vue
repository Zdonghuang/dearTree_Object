          ...
          <div class="tableBox d-shadow">
            ...
            >
              ...
              <el-table-column  align="center" label="期初">
                <el-table-column sortable align="center" show-overflow-tooltip property="initialquantity" label="数量"></el-table-column>
                <el-table-column sortable align="center" show-overflow-tooltip property="initialamount" label="余额">
                  <template slot-scope="scope">
                  <div class="tr">
                    <span v-if="scope.row.initialamount<0">-</span>
                    {{ $PublicJS.money(Math.abs(scope.row.initialamount), 2) }}
                  </div>
                </template>
                </el-table-column>
              </el-table-column>
              <el-table-column  align="center" label="入库">
                <el-table-column sortable align="center" show-overflow-tooltip property="inquantity" label="数量"></el-table-column>
                <el-table-column sortable align="center" show-overflow-tooltip property="inamount" label="成本金额">
                  <template slot-scope="scope">
                  <div class="tr">
                    <span v-if="scope.row.inamount<0">-</span>
                    {{ $PublicJS.money(Math.abs(scope.row.inamount), 2) }}
                  </div>
                </template>
                </el-table-column>
              </el-table-column>
              <el-table-column  align="center" label="出库">
                <el-table-column sortable align="center" show-overflow-tooltip property="outquantity" label="数量"></el-table-column>
                <el-table-column sortable align="center" show-overflow-tooltip property="outamount" label="成本金额">
                  <template slot-scope="scope">
                  <div class="tr">
                    <span v-if="scope.row.outamount<0">-</span>
                    {{ $PublicJS.money(Math.abs(scope.row.outamount), 2) }}
                  </div>
                </template>
                </el-table-column>
              </el-table-column>
              <el-table-column  align="center" label="期末">
                <el-table-column sortable align="center" show-overflow-tooltip property="endingquantity" label="期末数量"></el-table-column>
                <el-table-column sortable align="center" show-overflow-tooltip property="endingamount" label="期末金额">
                  <template slot-scope="scope">
                  <div class="tr">
                    <span v-if="scope.row.endingamount<0">-</span>
                    {{ $PublicJS.money(Math.abs(scope.row.endingamount), 2) }}
                  </div>
                </template>
                </el-table-column>
              </el-table-column>
...
export default {
  ...
  methods: {
    getList(val) {
      ...
      this.$api.Stat.statjxchzb(params).then(res => {
        loading.close();
        if (res.code == 200) {
