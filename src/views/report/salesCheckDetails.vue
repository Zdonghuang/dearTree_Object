  ...
  <div>
    ...
    <el-row :gutter="20">
      <el-col :sm="4" :xs="24">
        ...
        >
          <template slot="prefix">
            <span class="prefixSlot">制单人</span>
          </template>
          <el-option
            v-for="(item,index) in rHandmanOptions"
            :key="item.value"
            ...
            >
              ...
              >
                <template slot-scope="scope">
                  <div v-if="scope.row.orderTypeName=='销售调拨出库结算单'">{{scope.row.otherWarehouseName}}</div>
                </template>
              </el-table-column>
              <el-table-column
                show-overflow-tooltip
...
export default {
  ...
  created() {
    ...
    this.orderTypeOptions = [
      { value: 38, label: "销售退货结算单" },
      { value: 39, label: "销售换货结算单" },
      { value: 58, label: "盘点报废结算单" },
      { value: 37, label: "销售商品结算单" },
  ...
  methods: {
    getList(val) {
      ...
      if (!this.form.orderFrom) delete d.orderFrom;
      if (!this.form.updateDate) delete d.updateDate;
      if (!this.form.regUserId) delete d.regUserId;
      ...
      this.$api.ReportCapital.salesCheckDetails(d).then(res => {
    ...
    exportTable() {
      ...
      try {
        this.$fileSaver.saveAs(
          ...
          `销售核算明细列表${getDate.getToday().starttime}.xlsx`
        );
      } catch (e) {
        // 错误处理方式
      }
