    ...
    <el-row :gutter="20">
      ...
      <el-col :sm="6" :xs="24">
         <el-select
          ...
          placeholder="请选择仓库"
          size="small"
          @change="selectChange('whid',form.whidv)"
          @clear="clearInput('whid')"
          class="selectSlot2"
        >
          ...
          <el-option
            v-for="(item,index) in whidOptions"
            :key="item.value"
            :label="item.label"
            :value="item.value"
          >
  ...
  watch: {
    ...
    idList: {
      handler: function(n, o) {
        if (n) {
          this.form.whidv = this.$props.whidv;
          this.form.whid = this.$props.whid;
  ...
  methods: {
    ...
    getWhiid() {
      ...
      if (res.code == 200) {
        ...
        this.whidOptions = res.data.records.map(item => {
          return {
            label: item.whName,
            value: item.whId,
            whCode: item.whCode
